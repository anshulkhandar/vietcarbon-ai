...
Keep it under 120 words. Be specific, not generic.`;
  ...
  const sendAlertToCitizens = (alert) => {
    ...
    // Alerts are sent only through backend/database; no localStorage demo fallback.
    sendCitizenAlertFn?.(msg, alert.city, alert.type);
    setCitizenAlert(msg);
  };
...
export default function AdminDashboard({ initialTab }) {
  ...
  const sendCitizenAlert = async () => {
    ...
    try {
      ...
      alert('⚠ Alert not sent. Backend database/SMTP must be connected for real citizen alerts. No local demo alert was created.');
    }
  };
...
• Map action: select ${aiLocation} in Vietnam AI Map to zoom and highlight the affected zone.`);
  ...
  return (
    <div className="min-h-screen flex" style={{ background: '#020b05', color: '#e8f5e9', paddingRight: isFridayOpen ? 'calc(max(25%, 300px))' : '0', paddingLeft: sidebarOpen ? '224px' : '64px', transition: 'padding-right 0.3s, padding-left 0.3s' }}>
      {/* SIDEBAR - fixed so it doesn't scroll */}
      <aside className={`${sidebarOpen ? 'w-56' : 'w-16'} fixed top-0 left-0 h-screen flex flex-col transition-all duration-300 z-40`}
          ...
          {active === 'dataCenter' && (
            <div className="space-y-4">
              ...
              <Card>
                ...
                <form onSubmit={addManualRow} className="grid md:grid-cols-12 gap-3 items-center">
                  <select value={manualForm.city} onChange={e=>setManualForm({...manualForm,city:e.target.value})} className="p-2 rounded-lg text-xs font-mono md:col-span-2" style={{background:'#061209',border:'1px solid #00ff8822',color:'#e8f5e9'}}>{VIETNAM_CITIES.map(c=><option key={c.id}>{c.name}</option>)}</select>
                  <select value={manualForm.period} onChange={e=>setManualForm({...manualForm,period:e.target.value})} className="p-2 rounded-lg text-xs font-mono md:col-span-2" style={{background:'#061209',border:'1px solid #00ff8822',color:'#e8f5e9'}}><option>daily</option><option>monthly</option><option>yearly</option></select>
                  <input type="date" value={manualForm.date} onChange={e=>setManualForm({...manualForm,date:e.target.value})} className="p-2 rounded-lg text-xs font-mono md:col-span-2" style={{background:'#061209',border:'1px solid #00ff8822',color:'#e8f5e9'}} />
                  <select value={manualForm.metric} onChange={e=>setManualForm({...manualForm,metric:e.target.value})} className="p-2 rounded-lg text-xs font-mono md:col-span-2" style={{background:'#061209',border:'1px solid #00ff8822',color:'#e8f5e9'}}><option>CO2</option><option>Traffic</option><option>AQI</option><option>Renewable</option><option>Population</option><option>Flood Risk</option></select>
                  <div className="grid grid-cols-[minmax(160px,1fr)_auto] items-center gap-3 rounded-lg px-3 md:col-span-3" style={{background:'#061209',border:'1px solid #00ff8822',minWidth:0}}>
                    ...
                    <span className="whitespace-nowrap text-[11px] font-mono pl-2 border-l border-[#00ff8822]" style={{color:NEON2}}>{manualMetricUnit}</span>
                  </div>
                  <button className="p-2 rounded-lg text-xs font-mono font-bold md:col-span-1" style={{background:'linear-gradient(135deg,#00ff88,#00c853)',color:'#020b05'}}>ADD DATA</button>
                  <select value={manualForm.note} onChange={e=>setManualForm({...manualForm,note:e.target.value})} className="p-2 rounded-lg text-xs font-mono md:col-span-12" style={{background:'#061209',border:'1px solid #00ff8822',color:'#e8f5e9'}}>
